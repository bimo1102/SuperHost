...
import { MENU_LOCALSTORAGE_KEYS } from '@fc-aiot-fe-share/constant/common.constant';
import { MenuResponse } from '@fc-aiot-fe-share/types/menu.type';
import { deepMerge, getData } from '@fc-aiot-fe-share/utils';
import { createContext, type PropsWithChildren, useContext, useEffect, useState } from 'react';
import { useLocation } from 'react-router';
import { MainLayoutConfig } from './MainLayoutConfig';

// Interface defining the structure for layout provider properties
export interface IMainLayoutProviderProps {
...
const MainLayoutContext = createContext<IMainLayoutProviderProps>(initalLayoutProps);
...
const MainLayoutProvider = ({ children }: PropsWithChildren) => {
    const menuLocal = getData(MENU_LOCALSTORAGE_KEYS.MENU) as MenuResponse[];
    const { models } = useAppSelector((state) => state.menu);
    const finalModels = models && models.length > 0 ? models : menuLocal || [];
    const { pathname } = useLocation(); // Gets the current path
    const { setMenuConfig } = useMenus(); // Accesses menu configuration methods
    const secondaryMenu = useMenuChildren(pathname, finalModels, 0); // Retrieves the secondary menu

    // Sets the primary and secondary menu configurations
    setMenuConfig('primary', finalModels);
    ...
    const setSidebarCollapse = (collapse: boolean) => {
        ...
        updateLayout(MainLayoutConfig.name, updatedLayout); // Updates the layout with the collapsed state
