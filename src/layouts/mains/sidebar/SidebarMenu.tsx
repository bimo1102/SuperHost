...
const SidebarMenu = () => {
    ...
    const accordionLinkPl = 'ps-[15px]';
    const accordionLinkGap = ['gap-[10px]', 'gap-[5px]', 'gap-[5px]', 'gap-[5px]', 'gap-[5px]', 'gap-[5px]'];
    ...
    const buildMenuItemRoot = (item: MenuResponse, index: number) => {
        if (item.children) {
            return (
                <MenuItem
                    key={index}
    ...
    const buildMenuItemRootDisabled = (item: MenuResponse, index: number) => {
        return (
            <MenuItem key={index}>
                <MenuLabel className={clsx('border border-transparent', accordionLinkGap[0], linkPy, linkPl, linkPr)}>
                    ...
                    <MenuTitle className="text-sm font-medium text-gray-800">{t(item.titleKey)}</MenuTitle>
    ...
    const buildMenuItemChild = (item: MenuResponse, index: number, level: number = 0) => {
        if (item.children) {
            return (
                <MenuItem
                    key={index}
    ...
    const buildMenuItemChildDisabled = (item: MenuResponse, index: number, level: number = 0) => {
        return (
            <MenuItem key={index}>
                <MenuLabel
                    className={clsx(
                    ...
                    {/* {buildMenuBullet()} */}
                    {buildMenuIconOrBullet(item)}
                    <MenuTitle className="text-2sm font-normal text-gray-800">{t(item.titleKey)}</MenuTitle>
                    {item.disabled && buildMenuSoon()}
                </MenuLabel>
            </MenuItem>
        );
    ...
    // const buildMenuHeading = (item: MenuResponse, index: number) => {
    //     return (
    //         <MenuItem key={index} className="pt-2.25 pb-px">
    //             <MenuHeading className={clsx('uppercase ps-[10px] text-2sm font-medium text-gray-500', linkPl, linkPr)}>
    //                 {t(item.titleKey)}
    //             </MenuHeading>
    //         </MenuItem>
    //     );
    ...
    const buildMenuIconOrBullet = (item: MenuResponse) => {
        if (item.icon) {
            return (
                <MenuIcon
                    className={clsx(
